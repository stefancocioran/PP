...
(define (rotate left right Acc)
  (if (stream-empty? left)
      (if (null? right)
          Acc
          (rotate left (cdr (reverse right)) (stream-cons (car (reverse right)) Acc)))
      (if (null? right)
          (stream-append left Acc)
          (rotate left (cdr right) (stream-cons (car right) Acc)))))
...
(define (enqueue x q)
  (if  (<= (queue-size-l q) (queue-size-r q))
       (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)]
                                                         [right '()] [size-l (add1 (+ (queue-size-l q) (queue-size-r q)))] [size-r 0])
       (struct-copy queue q [right (cons x (queue-right q))] [size-r (add1 (queue-size-r q))])))
...
(define (dequeue q)
  (if (<= (queue-size-l q) (queue-size-r q))
      (struct-copy queue q [left (stream-rest (rotate (queue-left q) (queue-right q) empty-stream))]
                                                         [right '()] [size-l (sub1 (+ (queue-size-l q) (queue-size-r q)))] [size-r 0])
