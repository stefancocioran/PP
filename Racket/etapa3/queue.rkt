...
(define (enqueue x q)
  (define q-copy (struct-copy queue q [right (cons x (queue-right q))] [size-r (add1 (queue-size-r q))]))
  q-copy)
...
(define (dequeue q)
  ...
  (define (q-copy q) (if (= (queue-size-l q) 0)
                         q
                         (struct-copy queue q [left (cdr (queue-left q))] [size-l (- (queue-size-l q) 1)])))
  
  (define (q-left-null q) (if (= (queue-size-r q) 0)
                              q
                              (struct-copy queue q [left (cdr (reverse (queue-right q)))] [right '()] [size-l (- (length (queue-right q)) 1)] [size-r 0])))
...
(define (top q)
  ...
  (define (exception-top q) (if (= (queue-size-r q) 0)
                                '()
